...
const ProjectsSection = () => {
  ...
  const getSelectedTags = (tags: ITag[]): string[] => {
    return tags
      .map((tag: ITag) => (tag.selected ? tag._id : null))
  ...
  const toggleTagState = (
    ...
    let newViewAllState: boolean = toggledTag.isViewAll
      ? !toggledTag.selected
      : false
    ...
    const newSelectedTags: string[] = newViewAllState
      ? []
      : getSelectedTags(modifiedTags)
  ...
  const sessionStorageSelectedTags: string[] = isBrowser
    ? JSON.parse(sessionStorage.getItem("selectedTags"))
    : []
  const [tags, setTags] = useState(
    sessionStorageSelectedTags?.length
      ? allSanityTag.nodes.map((tag: ITag) => {
  ...
  const [selectedTags, setSelectedTags] = useState(
    sessionStorageSelectedTags?.length
      ? sessionStorageSelectedTags
      : getSelectedTags(tags)
  )
  const [viewAllSelected, setViewAllSelected] = useState(
    sessionStorageSelectedTags?.length ? false : true
  )
  ...
  useEffect(() => {
    setProjects(
      viewAllSelected
        ...
        : allSanityProject.nodes.filter((project: IProject) => {
            let includeProject: boolean = false
            for (
              let tagIndex: number = 0;
              tagIndex < project.tags.length;
              tagIndex++
            ) {
